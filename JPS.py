...
class JPS:
    ...
    # 检测 对于当前点坐标，（d_row, d_col)方向，是否存在强迫邻居 。在外侧考虑强迫邻居是否为有效节点
    def has_forced_neighbor(self, current_coord: Tuple[int, int], d_row, d_col) -> bool:
        ...
        # 当前点的前导节点 parent_row = current_row - d_row, parent_col = current_col - d_col
        # 考虑AGV 斜线通行条件 的 横向移动(与原JPS判断方法不同)
        ...
        # 考虑AGV 斜线通行条件 的 纵向移动(与原JPS判断方法不同)
        ...
        # 考虑AGV 斜线通行条件 的 斜向移动 不存在 强迫邻居 (与原JPS判断方法不同)
        """
    ...
    # 考虑AGV 斜线通行条件 对 由前导节点至当前节点 进行 剪枝 （与原JPS判断方法不同）
    def get_pruned_directions(self,
        ...
        # 否则 依据前导节点来推理 当前节点的有效方向
            ...
            _d_row = current_row - parent_row
            _d_col = current_col - parent_col
            ...
            # 建立剪枝后的方向集合
            raw = []
            # 斜线探索 一共有八个方向，我对标准JPS剪枝做了修改，削减离前导节点最近的三个方向，后继续消减两个方向
            # 保留至三个方向，两个正交直线、一个原先斜线，方向不受障碍物影响（我这里考虑的是AGV斜向通行，双侧无切角）
            if d_row != 0 and d_col != 0:
                ...
                # 为什么我后面两个方向，我注释掉了？因为要保证斜向通行，两侧需要同时无障碍物
                # 如果当前节点与父节点有一侧有障碍物，则 分枝后不可到达，如果没有障碍物，则其原先直线路径最短
                # 所有不管有没有障碍物，探索都没有意义
                # ====== 强迫邻居方向======
            ...
            # 直线横向探索 保留五个方向
                ...
                # 原来方向的横向移动
                raw.append((d_row, d_col))
                # 依据AGV斜向通行条件而加入的 两个垂直方向（与原JPS判断方法不同）
                raw.append((-1, 0))
                raw.append((1, 0))
                # 斜线分支1
                raw.append((-1, d_col))
    ...
    # 查询 当前节点 沿着dx、dy方向 移动后， 所有有效跳点 是否存在跳点，
    # 如果没有就继续移动，直到下一个移动点非法或者到达终点 ，其中调用了 检测强迫邻居函数
    def jump(self, current_coord: Tuple[int, int], d_row: int, d_col: int) -> Optional[Tuple[int, int]]:
        ...
        while True:
            ...
            # AGV斜线通行 两侧无障碍物 , 否则终止
            if d_row != 0 and d_col != 0:
                if (not self.is_valid_node((n_row, current_col))) or (not self.is_valid_node((current_row, n_col))):
                    return None
            # 到达终点，直接返回终点作为跳点
            ...
            if d_row != 0 and d_col != 0:
                ...
                if self.jump((n_row, n_col), 0, d_col) is not None:
                    return n_row, n_col
            # 倘若上述均无探测到跳点，继续向前走同一个方向
            current_row, current_col = n_row, n_col
        return None

    # 查询在有前导节点的当前节点情况下 所有有效的跳点
    def get_successors(self, current_coord: Tuple[int, int], parent_coord: Optional[Tuple[int, int]]
                       ) -> List[Tuple[int, int]]:
        """获取跳点后继集合"""
        ...
        for d_row, d_col in dirs:
            ...
            # 有效点判断
            if not self.is_valid_node((current_row + d_row, current_col + d_col)):
                continue
            # 斜向通行判断
            if d_row != 0 and d_col != 0:
                if (not self.is_valid_node((current_row + d_row, current_col))) or (
                not self.is_valid_node((current_row, current_col + d_col))):
                    continue
            # 查询当前点沿着dir方向 出发的跳点
            jp = self.jump(current_coord, d_row, d_col)

            # 如果跳点存在，则 加入到后继节点列表中
            if jp is not None: successors.append(jp)
    ...
    def search(self):
        ...
        source_node: GridNode = GridNode(self.source)

        open_list: List[GridNode] = [source_node]

        explored_set: Set[Tuple[int, int]] = set()

        open_dict = {source_node.coord: source_node}
        ...
        while open_list:
            ...
            for success_coord in successors:
                # 记录后继跳点信息，并实例化后，压入最优队列中
                d_row = success_coord[0] - current_node.coord[0]
                d_col = success_coord[1] - current_node.coord[1]
                cost = math.hypot(d_row, d_col)

                tentative_g = current_node.g_cost + cost

                existing = open_dict.get(success_coord)

                if existing is None or tentative_g < existing.g_cost:
                    successor_node = GridNode(success_coord)
                    successor_node.g_cost = tentative_g
                    successor_node.h_cost = self.heuristic(success_coord)
                    successor_node.parent = current_node
                    heapq.heappush(open_list, successor_node)
                    open_dict[success_coord] = successor_node
